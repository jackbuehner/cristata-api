...
router.get("/v3/:tenant/user-photo/:user_id", async (req, res) => {
  try {
    ...
    const user_id =
      req.params.user_id === "me" ? authUser._id : req.params.user_id;
    ...
    if (user?.photo) {
      ...
      const size =
        (req.query as unknown as URLSearchParams).get("size") || "120";

      const avatar = new URL(
        `https://source.boringavatars.com/beam/${size}/62fd3aef937c1ea557370d33`
      );
      ...
      const colorHash = new ColorHash({
        ...
        lightness: 0.7,
        hash: "bkdr",
      }); // note that this config is different than the one that picks the user accent color
...
router.get("/filestore/troop-370/:_id", (req, res) => {
  res.redirect(
    301,
    `https://troop370atlanta.org/cristata-filestore/${req.params._id}`
  );
});
...
router.get("/filestore/:tenant/:_id", async (req, res) => {
  try {
    ...
    const fileName =
      (foundFile.name.split(".").slice(0, -1) || "file") +
      "." +
      mime.extension(foundFile.file_type);
    res.setHeader("Content-Type", `${foundFile.file_type}; charset=UTF-8`);
    res.setHeader(
      "Content-Disposition",
      `inline; filename="${fileName.replace(/[^a-zA-Z-._\d\s:]/g, "_")}"`
    );
...
router.get("/photo/:tenant/:_id", async (req, res) => {
  try {
    ...
    // use the correct mime type and name
    const fileName =
      (foundPhoto.name.split(".").slice(0, -1) || "photo") +
      "." +
      mime.extension(foundPhoto.file_type);
    ...
    if (searchParams.has("tr")) {
      ...
      const width = transformations
        .find((tr) => tr.indexOf("w-") === 0)
        ?.replace("w-", "");
      const height = transformations
        .find((tr) => tr.indexOf("h-") === 0)
        ?.replace("h-", "");
      const resizeFit = transformations
        .find((tr) => tr.indexOf("fit-") === 0)
        ?.replace("fit-", "");
