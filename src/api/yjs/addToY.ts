...
function addToY(params: AddToYParams) {
  ...
  params.schemaDef.forEach(([key, def]) => {
      ...
      if (schemaType === 'DocArray') {
        ...
        if (def.docs) {
          generatedUuids.forEach((uuid, index) => {
            ...
            addToY({ ydoc: params.ydoc, schemaDef: namedSubdocSchemas, inputData: data });
          });
        }
      }
